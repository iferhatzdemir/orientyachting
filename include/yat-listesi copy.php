...
// Toplam yat sayısını ve özellikleri al
$toplam_yat = $VT->VeriGetir("yachts", "WHERE durum=1");
$toplam_sayfa = $toplam_yat ? ceil(count($toplam_yat) / $limit) : 0;

// İstatistikler için sorguları yap
$aktif_yat_sayisi = count($toplam_yat);

// Toplam kabin sayısı
$toplam_kabin = 0;
if($toplam_yat) {
    foreach($toplam_yat as $yat) {
    ...
    <div class="container">
        ...
        <div class="pagination-wrapper">
            <div class="pagination">
                ...
                <a href="?p=<?=$i?>" class="page-link <?=$i == $sayfa ? 'active' : ''?>"><?=$i?></a>
                <?php endfor; ?>
            </div>
        </div>
        <?php endif; ?>
