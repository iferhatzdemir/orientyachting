...
class VT{
	...
	if ($handle->uploaded) {
		...
		else if($tur=="video" || !empty($_FILES[$nesnename]["name"][0])) // Video türü için veya doğrudan dosya varsa
		{
			// Video dosyası yüklemeleri için özel işlem
			if(!empty($_FILES[$nesnename]["name"][0]))
			{
				// Logla
				error_log("Video yükleme denemesi");
				
				$dosyanizinadi=$_FILES[$nesnename]["name"][0];
				$tmp_name=$_FILES[$nesnename]["tmp_name"][0];
				$uzanti=$this->uzanti($dosyanizinadi);
				$izinliVideoUzantilari = ["mp4", "webm", "ogg", "mov", "avi"];
				...
				foreach ($videolar as $video){
					...
					if(in_array($uzanti, $izinliVideoUzantilari))
					{
						...
						// Hedef yolu oluştur
						$hedefYol = $yuklenecekyer . $yeniDosyaAdi;
						...
						// Dosyayı yükle
						if(move_uploaded_file($video["tmp_name"], $hedefYol)) {
							error_log("Video dosyası başarıyla yüklendi: " . $hedefYol);
							...
							error_log("Video yükleme hatası: " . $video["name"] . " - " . $video["tmp_name"] . " -> " . $hedefYol);
						}
					} else {
	...
	public function upload($nesnename,$yuklenecekyer='images/',$tur='img',$w='',$h='',$resimyazisi='')
	{
			...
			if(!empty($_FILES[$nesnename]["name"]))
			{
				...
				if($uzanti=="png" || $uzanti=="jpg" || $uzanti=="jpeg" || $uzanti=="gif")
				{
					...
					if($classIMG->uploaded)
					{
						...
						$classIMG->file_new_name_ext = $classIMG->image_src_type;
		...
		else if($tur=="video")
		{
			if(!empty($_FILES[$nesnename]["name"]))
			{
				...
				$tmp_name=$_FILES[$nesnename]["tmp_name"];
				$uzanti=$this->uzanti($dosyanizinadi);
				if($uzanti=="mp4" || $uzanti=="avi" || $uzanti=="mov" || $uzanti=="webm")
				{
					$classIMG=new upload($_FILES[$nesnename]);
					if($classIMG->uploaded)
					{
